...
import {actionDSHSCV} from '@redux/actions/congviec';
import DSCongViecItemComponent from './components/DSCongViecItemComponent';
import CTCongViec from '../CTDKXe';
...
const DSCongViecScreen = (props: Props) => {
		...
		if (dsCongViecResponse.success) {
			if (dsCongViecResponse.data && dsCongViecResponse.data?.length > 0) {
				...
				setParamDS({
					...paramDS,
					isLoadMore: false,
					needMore: dsCongViecResponse.data?.length == paramDS.pageInfo.pageSize,
					pageInfo,
				});
	...
	const renderItem = (item: any, index: number) => (
		<DSCongViecItemComponent key={index} item={item} onItemPress={_onDocumentItem} />
	);
